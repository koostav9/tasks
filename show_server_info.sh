#!/bin/bash

#===================================================================================================
# Apache Server Info 출력 스크립트
# 사용법: ./show_server_info.sh <인스턴스명>
#   인스턴스명: Apache 인스턴스 이름 (예: instance1, instance2)
#===================================================================================================

# 환경 설정 파일 로드
SCRIPT_DIR=$(dirname "${BASH_SOURCE[0]}")
ENV_FILE="${SCRIPT_DIR}/web_mon_cron.env"

if [ ! -f "$ENV_FILE" ]; then
    echo "Error: Environment file not found: $ENV_FILE"
    exit 1
fi

# 환경 변수 로드
source "$ENV_FILE"

# 사용법 출력 함수
usage() {
    echo "Usage: $0 <instance_name>"
    echo ""
    echo "Parameters:"
    echo "  instance_name    Apache instance name (e.g., instance1, instance2)"
    echo ""
    echo "Available instances from configuration:"
    echo "  Apache instances: ${APACHE_INST_NAME[@]}"
    echo ""
    echo "Examples:"
    echo "  $0 instance1                # Show server-info for instance1"
    echo "  $0 instance2                # Show server-info for instance2"
    exit 1
}

# 인스턴스 인덱스 찾기 함수
find_instance_index() {
    local instance_name="$1"
    local instance_array=("${@:2}")
    
    for i in "${!instance_array[@]}"; do
        if [ "${instance_array[$i]}" = "$instance_name" ]; then
            echo $i
            return 0
        fi
    done
    echo -1
}

# Apache server-info 출력 함수
show_apache_server_info() {
    local instance_name="$1"
    local instance_index=$(find_instance_index "$instance_name" "${APACHE_INST_NAME[@]}")

    if [ "$instance_index" -eq -1 ]; then
        return 1
    fi

    local https_port="${APACHE_HTTPS_PORT[$instance_index]}"
    local http_port="${APACHE_HTTP_PORT[$instance_index]}"

    # HTTPS 포트가 설정되어 있으면 HTTPS를 우선 시도
    if [ "$https_port" != "-" ] && [ -n "$https_port" ]; then
        local info_url="https://${https_port}/server-info"
        echo "Server Info URL (HTTPS): $info_url"
        echo ""

        # curl 명령어 존재 여부 확인
        if command -v curl >/dev/null 2>&1; then
            # curl을 사용하여 HTTPS server-info 요청
            if curl -s --connect-timeout 10 --max-time 30 -k \
                   -H "User-Agent: Apache-Monitor-Script" \
                   "$info_url" 2>/dev/null; then
                return 0
            fi
        elif command -v wget >/dev/null 2>&1; then
            # wget을 사용하여 HTTPS server-info 요청 (curl이 없는 경우)
            if wget -q --timeout=30 --tries=1 --no-check-certificate \
                   --user-agent="Apache-Monitor-Script" \
                   -O - "$info_url" 2>/dev/null; then
                return 0
            fi
        fi

        echo "Warning: HTTPS request failed, trying HTTP..."
        echo ""
    fi

    # HTTP 포트로 시도 (HTTPS가 실패했거나 설정되지 않은 경우)
    if [ "$http_port" != "-" ] && [ -n "$http_port" ]; then
        local info_url="http://${http_port}/server-info"
        echo "Server Info URL (HTTP): $info_url"
        echo ""

        # curl 명령어 존재 여부 확인
        if command -v curl >/dev/null 2>&1; then
            # curl을 사용하여 HTTP server-info 요청
            if curl -s --connect-timeout 10 --max-time 30 \
                   -H "User-Agent: Apache-Monitor-Script" \
                   "$info_url" 2>/dev/null; then
                return 0
            else
                echo "Error: Failed to retrieve server-info from $info_url"
                echo "Possible causes:"
                echo "- Apache server is not running"
                echo "- server-info module is not enabled"
                echo "- Network connectivity issues"
                echo "- Access restrictions on server-info"
                echo "- HTTPS redirect is enforced (check HTTPS configuration)"
                return 1
            fi
        elif command -v wget >/dev/null 2>&1; then
            # wget을 사용하여 HTTP server-info 요청 (curl이 없는 경우)
            if wget -q --timeout=30 --tries=1 \
                   --user-agent="Apache-Monitor-Script" \
                   -O - "$info_url" 2>/dev/null; then
                return 0
            else
                echo "Error: Failed to retrieve server-info from $info_url"
                echo "Possible causes:"
                echo "- Apache server is not running"
                echo "- server-info module is not enabled"
                echo "- Network connectivity issues"
                echo "- Access restrictions on server-info"
                echo "- HTTPS redirect is enforced (check HTTPS configuration)"
                return 1
            fi
        else
            echo "Error: Neither curl nor wget is available for HTTP requests"
            return 1
        fi
    else
        echo "Error: Neither HTTP nor HTTPS port configured for instance: $instance_name"
        return 1
    fi
}


# 메인 스크립트 로직
main() {
    # 파라미터 검증
    if [ $# -ne 1 ]; then
        usage
    fi
    
    local instance_name="$1"
    
    echo "====================================================================================================="
    echo "Instance: $instance_name"
    
    # 인스턴스 존재 여부 확인
    local instance_index=$(find_instance_index "$instance_name" "${APACHE_INST_NAME[@]}")
    if [ "$instance_index" -eq -1 ]; then
        echo "Error: Apache instance '$instance_name' not found in configuration"
        echo "Available instances: ${APACHE_INST_NAME[@]}"
        echo "====================================================================================================="
        return 1
    fi
    
    echo "Apache Server Info for Instance: $instance_name"
    echo "====================================================================================================="
    
    # server-info 출력
    if ! show_apache_server_info "$instance_name"; then
        echo ""
        echo "Unable to retrieve server-info for instance: $instance_name"
    fi
    
    echo "====================================================================================================="
    echo "Info display completed."
}

# 메인 함수 실행
main "$@"