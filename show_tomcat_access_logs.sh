...
#===================================================================================================
# Tomcat Access 로그 출력 스크립트
# 사용법: ./show_tomcat_access_logs.sh <인스턴스명>
...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 instance1                # Show access logs for instance1"
    echo "  $0 instance2                # Show access logs for instance2"
    exit 1
}
...
# Tomcat Access 로그 출력 함수
show_tomcat_access_logs() {
    local instance_name="$1"
    ...
    # 최신 access log 파일 찾기
    local recent_access_log=$(ls -t "${instance_log_dir}"/localhost_access.*.log 2>/dev/null | head -1)
    
    if [ -n "$recent_access_log" ] && [ -f "$recent_access_log" ]; then
        echo "Recent Access log file: $(basename "$recent_access_log")"
        echo "File size: $(du -h "$recent_access_log" | cut -f1 2>/dev/null || echo "Unknown")"
        echo ""
        tail -100 "$recent_access_log"
        return 0
    else
        echo "No access log files found in directory: $instance_log_dir"
        echo "Expected format: localhost_access.*.log"
        return 1
    fi
}
...
main() {
    ...
    echo "Tomcat Access Log for Instance: $instance_name"
    echo "====================================================================================================="
    
    # Access 로그 출력
    if ! show_tomcat_access_logs "$instance_name"; then
