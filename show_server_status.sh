...
#===================================================================================================
# Apache Server Status 출력 스크립트
# 사용법: ./show_server_status.sh <인스턴스명>
...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 instance1                # Show server-status for instance1"
    echo "  $0 instance2                # Show server-status for instance2"
    exit 1
}
    ...
    if [ "$http_port" != "-" ] && [ -n "$http_port" ]; then
        ...
        if command -v curl >/dev/null 2>&1; then
            ...
            if curl -s --connect-timeout 10 --max-time 30 \
                ...
                echo "Error: Failed to retrieve server-status from $status_url"
...
main() {
    ...
    echo "Apache Server Status for Instance: $instance_name"
    echo "====================================================================================================="
    
    # server-status 출력
    if ! show_apache_server_status "$instance_name"; then
        echo ""
        echo "Unable to retrieve server-status for instance: $instance_name"
    fi
